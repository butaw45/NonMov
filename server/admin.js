...
function validateProviderFields(data, { strict = false } = {}) {
  // self-hosted
...
export function createEntry({ type, tmdb_id, title, status, video_url, video_type, episodes, video_provider, providers }) {
  ...
  if (hasProviders) {
    ...
    const validated = validateProviderFields(
      { type, tmdb_id, video_url, video_type, video_provider },
      { strict: true }
    )
...
export function updateEntry(id, patch) {
  ...
  let validated = null
  if (hasFlatPatch) validated = validateProviderFields(merged, { strict: true })
